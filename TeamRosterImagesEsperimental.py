...
class rosterimages:
    ...
    def dir_scan(self):
        ...
        if visible_bool is True and ((hud_data['Event Num'] == '0b')
            ...
            self.set_visible()
            transtion_scene = S.obs_get_source_by_name(self.transition_scene)
            S.obs_frontend_set_current_scene(transtion_scene)
            print("visible")
...
def script_properties():
    ...
    transition_scene = S.obs_properties_add_list(props, '_transition_scene', 'Transition Scene:', S.OBS_COMBO_TYPE_LIST, S.OBS_COMBO_FORMAT_STRING)
    scenes = S.obs_frontend_get_scene_names()
    for scene in scenes:
        S.obs_property_list_add_string(transition_scene, str(scene), str(scene))
...
def scene_callback(props, prop, settings):
    getimage.transition_scene = S.obs_data_get_string(settings, '_transition_scene')
    return True